...
export class UserComponent implements OnInit {
  ...
  new: boolean = false;
  id: string;
  constructor(private _UsersService: UsersService, private router: Router, private route: ActivatedRoute) {
    this.route.params.subscribe(parametros => {
      console.log(parametros);
      this.id = parametros['id'];
      if (this.id != 'nuevo') {
        this._UsersService.getUser(this.id).subscribe((user: User) => (this.user = user));
      }
    });
  }
